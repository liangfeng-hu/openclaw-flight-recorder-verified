...
- CLI: --input / --out / --policy-sim / --overwrite
- RFC-001 JSONL input (uses details.* + declared boolean)
- Outputs: badge.json + receipts.jsonl (hash-chained)

Enhancements (7 items):
1) Schema validation -> explicit EVIDENCE_GAP highlight
2) Unknown event_type -> explicit UNKNOWN_EVENT_TYPE highlight
3) Extended event types: DATABASE_OP / API_CALL / MEMORY_ACCESS
4) Advisory policy profiles + JSON config (policy.json)
5) Built-in receipt chain verifier: --verify-receipts
6) Optional anchor file (final_receipt_hash + optional HMAC via FLIGHT_ANCHOR_KEY)
7) Optional HTML report: --html
...
KNOWN_EVENT_TYPES = {
    ...
    "DATABASE_OP", "API_CALL", "MEMORY_ACCESS",   # extensions
    "EVIDENCE_GAP",                               # synthetic
}
...
DEFAULT_POLICY_PROFILES = {
    # Keep conformance: risky_run => 7 violations
    "advisory": {
...
def maybe_write_html(out_dir: str, badge: Dict[str, Any], html_flag: bool) -> Optional[str]:
    ...
    rows.append("<h1>Flight Recorder Report</h1>")
    rows.append(f"<p><b>Status</b>: {esc(badge.get('status'))}</p>")
    rows.append(f"<p><b>Total</b>: {esc(stats.get('total_events'))} | <b>Highlights</b>: {esc(stats.get('highlight_count'))} | <b>Gaps</b>: {esc(stats.get('evidence_gaps'))}</p>")
    ...
    for h in highlights:
        rows.append(f"<li>{esc(h.get('tag'))} (seq={esc(h.get('seq'))})
